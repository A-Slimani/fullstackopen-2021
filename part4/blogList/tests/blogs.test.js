...
describe('total likes', () => {
  test('blogs with no values', () => {
    expect(listHelper.totalLikes(emptyList)).toBe(0);
  });

  test('blogs with single entry', () => {
    expect(listHelper.totalLikes(oneBlogList)).toBe(7);
  });

  test('blogs with multiple entries', () => {
    expect(listHelper.totalLikes(multipleBlogList)).toBe(36);
  });
});

test('blog with most likes', () => {
  expect(listHelper.favouriteBlog(multipleBlogList)).toEqual(multipleBlogList[2]);
});
